create_inflow_outflow_file <- function(full_time_day,workingGLM,input_tz = 'EST5EDT', output_tz = reference_tzone){
  
  full_time_day_2017 <- as.POSIXct(full_time_day, tz = reference_tzone) - 365*24*60*60
  full_time_day_2016 <- as.POSIXct(full_time_day, tz = reference_tzone) - 2*365*24*60*60
  full_time_day_2015 <- as.POSIXct(full_time_day, tz = reference_tzone) - 3*365*24*60*60
  full_time_day_2014 <- as.POSIXct(full_time_day, tz = reference_tzone) - 4*365*24*60*60
  full_time_day_2015 <- as.POSIXct(full_time_day, tz = reference_tzone) - 5*365*24*60*60
  ...
  write.csv(inflow_new,file = paste0(workingGLM,'/','FCR_inflow.csv'),row.names = FALSE,quote = FALSE)
  write.csv(spillway_new,file = paste0(workingGLM,'/','FCR_spillway_outflow.csv'),row.names = FALSE,quote = FALSE)
}